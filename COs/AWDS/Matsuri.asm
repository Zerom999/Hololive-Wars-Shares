        setCODefaults           coAWDSMatsuri
	setCOGameID		coAWDSMatsuri,Game_AWDS

	setCOGFXFullbody        coAWDSMatsuri,GraphicsAWDSMatsuriGraphic
	setCOGFXName            coAWDSMatsuri,GraphicsAWDSMatsuriName
	setCOGFXPalette         coAWDSMatsuri,GraphicsAWDSMatsuriPalette
	setCOGFXFace            coAWDSMatsuri,GraphicsAWDSMatsuriFace,GraphicsAWDSMatsuriHappyFace,GraphicsAWDSMatsuriSadFace
	setCOGFXSmallFace       coAWDSMatsuri,GraphicsAWDSMatsuriMugshot
	setCOGFXPowerAnim       coAWDSMatsuri,0x04,0x00
	setCOGFXPowerSFX        coAWDSMatsuri,0xba,0xbb
	setCOGFXSuperAnim       coAWDSMatsuri,7,7
	setCOGFXSuperSFX        coAWDSMatsuri,0x1f7,0x1f8
	
	setCOName               coAWDSMatsuri,ScriptAWDSMatsuri
	setCOpowerName          coAWDSMatsuri,ScriptAWDSMatsuri+1
	setCOsuperName          coAWDSMatsuri,ScriptAWDSMatsuri+2
	setCOpowerText          coAWDSMatsuri,ScriptAWDSMatsuri+8,ScriptAWDSMatsuri+9,ScriptAWDSMatsuri+10,ScriptAWDSMatsuri+11,ScriptAWDSMatsuri+12,ScriptAWDSMatsuri+13
	setCOprofileText        coAWDSMatsuri,ScriptAWDSMatsuri+3,ScriptAWDSMatsuri+4,ScriptAWDSMatsuri+5,ScriptAWDSMatsuri+6
	setCOvictoryText        coAWDSMatsuri,ScriptAWDSMatsuri+7

	setCOFaction		coAWDSMatsuri,ArmyGreenEarth
	setCOMusic          	coAWDSMatsuri,MusicMatsuriTheme

	setCOPowerCost          coAWDSMatsuri,3
	setCOSuperCost          coAWDSMatsuri,6
	
	setCOD2DSoldierStat     coAWDSMatsuri,10,0,0,0
	setCOD2DLandStat	coAWDSMatsuri,-20,0,0,0
	setCOD2DNavyStat        coAWDSMatsuri,-20,0,0,0
	setCOD2DTransportStat	coAWDSMatsuri,0,0,1,0	
	setCOD2DUnitStat	coAWDSSensei,Unit_BattleCopter,50,0,0,0	
	
	setCOPowerSoldierStat	coAWDSMatsuri,30,0,1,0
	setCOPowerLandStat	coAWDSMatsuri,0,0,0,0
	setCOPowerNavyStat      coAWDSMatsuri,0,0,0,0
	setCOPowerTransportStat	coAWDSMatsuri,0,0,2,0
	setCOPowerUnitStat	coAWDSMatsuri,Unit_BattleCopter,100,0,1,0	

	setCOSuperSoldierStat	coAWDSMatsuri,50,0,2,0
	setCOSuperLandStat	coAWDSMatsuri,0,0,0,0
	setCOSuperNavyStat      coAWDSMatsuri,0,0,0,0
	setCOSuperTransportStat	coAWDSMatsuri,0,0,3,0
	setCOSuperUnitStat	coAWDSMatsuri,Unit_BattleCopter,150,0,2,0	

	setCOGFXPowerFX		coAWDSMatsuri,COPEffectResupplyUnit
	setCOGFXSuperFX		coAWDSMatsuri,COPEffectResupplyUnit

	setCOAIPowerRange       coAWDSJess,25
	setCOAIPowerTrigger     coAWDSJess,95
; blank line
