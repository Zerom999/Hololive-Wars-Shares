	setCODefaults           coAWDSJavier
	setCOGameID		coAWDSJavier,Game_AWDS

	setCOGFXFullbody        coAWDSJavier,GraphicsAWDSJavierGraphic
	setCOGFXName            coAWDSJavier,GraphicsAWDSJavierName
	setCOGFXPalette         coAWDSJavier,GraphicsAWDSJavierPalette
	setCOGFXFace            coAWDSJavier,GraphicsAWDSJavierFace,GraphicsAWDSJavierHappyFace,GraphicsAWDSJavierSadFace
	setCOGFXSmallFace       coAWDSJavier,GraphicsAWDSJavierMugshot
	setCOGFXPowerAnim       coAWDSJavier,3,1
	setCOGFXPowerSFX        coAWDSJavier,0xb7,0xb8
	setCOGFXSuperAnim       coAWDSJavier,7,7
	setCOGFXSuperSFX        coAWDSJavier,0x1f7,0x1f8
	
	setCOName               coAWDSJavier,ScriptAWDSJavier
	setCOpowerName          coAWDSJavier,ScriptAWDSJavier+1
	setCOsuperName          coAWDSJavier,ScriptAWDSJavier+2
	setCOpowerText          coAWDSJavier,ScriptAWDSJavier+8,ScriptAWDSJavier+9,ScriptAWDSJavier+10,ScriptAWDSJavier+11,ScriptAWDSJavier+12,ScriptAWDSJavier+13
	setCOprofileText        coAWDSJavier,ScriptAWDSJavier+3,ScriptAWDSJavier+4,ScriptAWDSJavier+5,ScriptAWDSJavier+6
	setCOvictoryText        coAWDSJavier,ScriptAWDSJavier+7

	setCOFaction		coAWDSJavier,ArmyGreenEarth
	setCOMusic          	coAWDSJavier,MusicGreenEarth

	setCOPowerCost          coAWDSJavier,3
	setCOSuperCost          coAWDSJavier,6

	setCOd2dAbilities		coAWDSJavier,SpecialTowerDefence
	setCOD2DIndirectDefence		coAWDSJavier,30
	setCOPowerAbilities		coAWDSJavier,SpecialTowerDefence+SpecialTowerDouble
	setCOPowerIndirectDefence	coAWDSJavier,60
	setCOSuperAbilities		coAWDSJavier,SpecialTowerDefence+SpecialTowerTriple
	setCOSuperIndirectDefence	coAWDSJavier,90
	
	setCOAIPowerRange       coAWDSJavier,25
	setCOAIPowerTrigger     coAWDSJavier,95
; blank line
