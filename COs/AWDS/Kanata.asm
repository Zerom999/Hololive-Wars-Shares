	setCODefaults           coAWDSKanata
	setCOGameID		coAWDSKanata,Game_AWDS

	setCOGFXFullbody        coAWDSKanata,GraphicsAWDSAndyGraphic
	setCOGFXName            coAWDSKanata,GraphicsAWDSAndyName
	setCOGFXPalette         coAWDSKanata,GraphicsAWDSAndyPalette
	setCOGFXFace            coAWDSKanata,GraphicsAWDSAndyFace,GraphicsAWDSAndyHappyFace,GraphicsAWDSAndySadFace
	setCOGFXSmallFace       coAWDSKanata,GraphicsAWDSAndyMugshot
	setCOGFXPowerAnim       coAWDSKanata,5,0
	setCOGFXPowerSFX        coAWDSAndy,0xb4,0xb5
	setCOGFXSuperAnim       coAWDSAndy,7,7
	setCOGFXSuperSFX        coAWDSAndy,0x1f7,0x1f8
	
	setCOName               coAWDSAndy,ScriptAWDSAndy
	setCOpowerName          coAWDSAndy,ScriptAWDSAndy+1
	setCOsuperName          coAWDSAndy,ScriptAWDSAndy+2
	setCOpowerText          coAWDSAndy,ScriptAWDSAndy+8,ScriptAWDSAndy+9,ScriptAWDSAndy+10,ScriptAWDSAndy+11,ScriptAWDSAndy+12,ScriptAWDSAndy+13
	setCOprofileText        coAWDSAndy,ScriptAWDSAndy+3,ScriptAWDSAndy+4,ScriptAWDSAndy+5,ScriptAWDSAndy+6
	setCOvictoryText        coAWDSAndy,ScriptAWDSAndy+7

	setCOFaction		coAWDSAndy,ArmyOrangeStar
	setCOMusic          	coAWDSAndy,MusicAndyTheme

	setCOPowerCost          coAWDSAndy,3
	setCOSuperCost          coAWDSAndy,6

	setCOSuperAllStat	coAWDSAndy,30,0,1,0

	setCOGFXPowerFX         coAWDSandy,COPEffectHealThreeHP
	setCOGFXSuperFX         coAWDSandy,COPEffectHealSixHP
	
	setCOAIPowerRange       coAWDSAndy,25
	setCOAIPowerTrigger     coAWDSAndy,95
	setCOAIPowerTriggerASM  coAWDSandy,AIActivateIfInjured

; blank line
